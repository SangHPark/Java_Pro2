	...
	public static void main(String[] args) {
		try {
			...
			for (int t = 1; t < T + 1; t ++ ) {
				...
				result = 0;
				System.out.println("result : " + myDFS(new myPOS(0, 0, 0, false, false)));
	...
	static int myDFS(myPOS curr) {
		...
		if (curr.x + 1 < M && !curr.onlyRight) {
			if (city_map[curr.x + 1][curr.y] == 0) {
				myDFS(new myPOS(curr.x + 1, curr.y, city_map[curr.x + 1][curr.y], false, false));
			} else if(city_map[curr.x + 1][curr.y] == 2) {
				myDFS(new myPOS(curr.x + 1, curr.y, city_map[curr.x + 1][curr.y], false, true));
			}
		}
		...
		if (curr.y + 1 < N && !curr.onlyDown) {
			if (city_map[curr.x][curr.y + 1] == 0) {
				myDFS(new myPOS(curr.x, curr.y + 1 , city_map[curr.x][curr.y + 1], false, false));
			} else if(city_map[curr.x][curr.y + 1] == 2) {
				myDFS(new myPOS(curr.x, curr.y + 1, city_map[curr.x][curr.y + 1], true, false));
			}
		}
